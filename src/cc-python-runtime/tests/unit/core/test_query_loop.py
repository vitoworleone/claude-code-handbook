...
from cc.models.content_blocks import TextBlock, ThinkingBlock, ToolUseBlock
...
class TestQueryLoopTextOnly:
    ...
    async def test_preserves_api_content_blocks_in_transcript(self) -> None:
        """TurnComplete.content is the authoritative assistant message content."""
        registry = ToolRegistry()
        messages: list[Message] = [UserMessage(content="scan")]

        api_content = [
            ThinkingBlock(thinking="Need to inspect files.", signature="sig_think_1"),
            TextBlock(text="I'll inspect the README."),
            ToolUseBlock(id="tu_1", name="MissingTool", input={}),
        ]

        async def mock_call_model(**kwargs: Any) -> Any:
            yield TextDelta(text="I'll inspect the README.")
            yield ToolUseStart(tool_name="MissingTool", tool_id="tu_1", input={})
            yield TurnComplete(stop_reason="tool_use", usage=Usage(), content=api_content)
        ...
        assert assistant_messages[0].content == api_content
        assert assistant_messages[0].to_api_dict()["content"][0] == {
            "type": "thinking",
