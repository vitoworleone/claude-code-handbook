...
from cc.core.events import TextDelta, ThinkingDelta, ToolUseStart, TurnComplete
from cc.models.content_blocks import ThinkingBlock, ToolUseBlock
...
def make_thinking_tool_use_stream_events() -> list[MockEvent]:
    """Create mock events for a thinking + tool_use response."""
    return [
        MockEvent(type="message_start", message=MockMessage(usage=MockUsage())),
        MockEvent(type="content_block_start", index=0, content_block=MockContentBlock(type="thinking")),
        MockEvent(
            type="content_block_delta",
            index=0,
            delta=MockDelta(type="thinking_delta", thinking="Need to inspect files."),
        ),
        MockEvent(
            type="content_block_delta",
            index=0,
            delta=MockDelta(type="signature_delta", signature="sig_think_1"),
        ),
        MockEvent(type="content_block_stop", index=0),
        MockEvent(
            type="content_block_start",
            index=1,
            content_block=MockContentBlock(type="tool_use", id="tu_1", name="Read"),
        ),
        MockEvent(
            type="content_block_delta",
            index=1,
            delta=MockDelta(type="input_json_delta", partial_json='{"file_path": "README.md"}'),
        ),
        MockEvent(type="content_block_stop", index=1),
        MockEvent(
            type="message_delta",
...
class TestStreamResponseThinking:
    async def test_turn_complete_preserves_thinking_blocks(self) -> None:
        """Thinking content must be available for transcript replay after tool calls."""
        mock_client = MagicMock()
        mock_client.messages.stream = MagicMock(return_value=MockStream(make_thinking_tool_use_stream_events()))
        ...
        assert thinking_deltas[0].text == "Need to inspect files."

        turns = [e for e in events if isinstance(e, TurnComplete)]
        assert len(turns) == 1
        assert turns[0].content is not None
        assert isinstance(turns[0].content[0], ThinkingBlock)
        assert turns[0].content[0].thinking == "Need to inspect files."
        assert turns[0].content[0].signature == "sig_think_1"
        assert isinstance(turns[0].content[1], ToolUseBlock)
        assert turns[0].content[1].id == "tu_1"
